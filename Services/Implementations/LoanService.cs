    ...
    public class LoanService
    {
        ...
        public async Task<LoanVM> AddAsync(LoanVM entity)
        {
            ...
            var loan = new Loan
            {
                ...
                Customer = _mapper.Map<Customer>(entity.CustomerVM),
        ...
        {
            var loans = await _repo.GetAllAsync();
            return loans.Select(item => new LoanVM()
            {
                CustomerVM = _mapper.Map<CustomerVM>(item.Customer),
                Employee = _mapper.Map<EmployeeVM>(item.Employee),
                LoanDetailVM = _mapper.Map<LoanDetailVM>(item.LoanDetail),
                LoanItemVMs = _mapper.Map<ICollection<LoanItemVM>>(item.LoanItems),
                EmployeeId = item.EmployeeId,
                Id = item.Id,
                LoanAmount = item.LoanAmount,
                PaymentVMs = _mapper.Map<ICollection<PaymentVM>>(item.Payments),
